...
export PATH=${PWD}/../bin:${PWD}:/home/zghh/go/src/github.com/zghh/redactable-fabric/release/linux-amd64/bin:$PATH
...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(docker run --rm zghh/redactable-fabric-tools:$IMAGETAG peer version | sed -ne 's/ Version: //p' | head -1)
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/first-network --rm zghh/redactable-fabric-tools:$IMAGETAG rm -Rf /tmp/first-network/ledgers-backup
